...
export default function TradesPage({ currentUser, onUserUpdate }) {
  ...
  const loadAll = async () => {
    const [targetsRes, mineRes, inboxRes, sentRes] = await Promise.all([
      ...
      api.get("/listings", { params: { seller: currentUser } }),
      api.get("/trades", { params: { user: currentUser, role: "inbox" } }),
      api.get("/trades", { params: { user: currentUser, role: "sent" } }),
    ]);

    const allActive = targetsRes.data || [];
    setAvailableTargets(allActive.filter((l) => l.sellerName?.toLowerCase() !== currentUser?.toLowerCase()));
  ...
  const submit = async (e) => {
    ...
    try {
      await api.post("/trades", {
        ...
        offeredListingIds: selectedOffered.map(Number),
      });
      setSelectedOffered([]);
      await loadAll();
  ...
  const accept = async (id) => {
    try {
      await api.put(`/trades/${id}/accept`, null, { params: { user: currentUser } });
      await loadAll();
      await onUserUpdate?.();
    } catch (err) {
      alert(err.response?.data?.message || err.message);
  ...
  const reject = async (id) => {
    try {
      await api.put(`/trades/${id}/reject`, null, { params: { user: currentUser } });
      await loadAll();
    } catch (err) {
      alert(err.response?.data?.message || err.message);
    }
